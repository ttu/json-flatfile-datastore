...
namespace JsonFlatFileDataStore
{
    public class DataStore : IDataStore
    {
            ...
            string encryptionKey = null, bool minifyJson = false)
        {
            ...
            _toJsonFunc = useLowerCamelCase
                ? new Func<JsonElement, string>(data =>
                {
                    ...
                    var test = JsonSerializer.Serialize(expandoObject, _serializerOptions);
                    // FIX: System.Text.Json doesn't automatically remove duplicates, e.g. age and Age
                    // test: UpdateOneAsync_DynamicUser_WrongCase
                    return test;
                })
        ...
        public IDocumentCollection<T> GetCollection<T>(string name = null) where T : class
        {
            ...
            // FIX: Datetime format should be in the format that System.Text.Json expects
            // test: FullTextSearch_Typed
   ...
   (!property.EnumerateArray().Any() || property.EnumerateArray().First().ValueKind == JsonValueKind.Object);
        ...
        private dynamic SingleDynamicItemReadConverter(JsonElement e)
        {
            switch (e.ValueKind)
            {
                ...
                case JsonValueKind.String:
                    ...
                    // Performance Note: DateTime.TryParse() is called for every string value,
                    // which has a performance cost. This maintains backward compatibility with
                    // Newtonsoft.Json's automatic date parsing behavior.
                    var strValue = e.GetString();
                    if (!string.IsNullOrEmpty(strValue) && DateTime.TryParse(strValue, out DateTime dateTime))
                    {
                        return dateTime;
                    }
                    return strValue;
        ...
        private JsonElement ConvertToJsonElement(object item)
        {
            ...
            // Return the root element (JsonElement) of the JsonDocument
        ...
        public (bool, JsonElement) RemoveJsonDataElement(JsonElement original, string key)
        {
            ...
            var modifiedJson = JsonSerializer.Serialize(jsonDataDict);
            return (removed, JsonDocument.Parse(modifiedJson).RootElement);
        }
